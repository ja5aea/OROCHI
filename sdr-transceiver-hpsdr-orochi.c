...
21.02.2021 JA5AEA: add RX1 and RX2 S-ATT PE4302 (SPI), TS-680S CAT control via ttyUSB0, Audio Codec to run Audio Codec at 12.000MHz and LINE-IN

/*added spidev.h and types.h JA5AEA*/
#include <stdio.h>
#include <errno.h>
#include <stdlib.h>
#include <limits.h>
...
/*end: added spidev.h and types.h JA5AEA*/
...
/*added for USB serial and spi_fd de JA5AEA*/
int usb_serial, spi_fd;
/*end: added for USB serial and spi_fd de JA5AEA*/
int receivers = 1;
int rate = 0;
...
/*added JA5AEA*/
uint8_t i2c_linein_data = 0;
uint8_t i2c_linevol_data = 0;
/*added JA5AEA*/
...
/* set USB serial PTT de JA5AEA*/
uint8_t ptt_data = 0;
/* end: set USB serial PTT de JA5AEA*/
...
int main(int argc, char *argv[])
{
  ...
  uint8_t chan = 0;
  long number;
/* added 2 more arguments for CAT and CODEC clocks JA5AEA*/
...
In Thetis (ANAN*D modes), RX1 is shown as RX1, RX2 is used for diversity, RX3 is shown as RX2, RX4 is used for Pure Signal.*/
  ...
  for(i = 0; i < 7; ++i)
  {
    ...
    {
        /* added two 1|2 JA5AEA*/
      printf("Usage: sdr-transceiver-hpsdr 1|2 1|2 1|2 1|2 1|2 1|2 1|2 1|2\n");
      return EXIT_FAILURE;
    }
    chan |= (number - 1) << i;
  }
 /* end: added 2 more arguments for CAT and CODEC clocks JA5AEA*/
  ...
  /*USB serial and SPI initial setting JA5AEA*/
  /* USB serial initial setting JA5AEA*/
  if ((usb_serial = open("/dev/ttyUSB0", O_RDWR | O_NOCTTY | O_NDELAY)) >= 0)
  {
      tcgetattr(usb_serial, &tty);
      /*changed to 7 JA5AEA*/
      switch (atoi(argv[7])) {
      case 1: /* 4800 bps 8bit 2stop bit */
          cfsetspeed(&tty, (speed_t)B4800);
          tty.c_cflag |= CSTOPB; /* set two stop bits */
          printf("Kenwood CAT 4,800bps  selected\n");
          break;
      case 2: /* 38400bit 8bit 1stop bit */
          cfsetspeed(&tty, (speed_t)B38400);
          tty.c_cflag &= ~CSTOPB; /* one stop bits */
          printf("Kenwood CAT 38,400bps  selected\n");
          break;
      default:
          printf("Usage: undefined last argement\n");
          return EXIT_FAILURE;
          break;
      }
      cfmakeraw(&tty);
      tty.c_cflag &= ~CRTSCTS; /* (not in POSIX) diable RTS/CTS flow control */
      ...
      tcsetattr(usb_serial, TCSANOW, &tty);
  }

  /*SPI initial setting JA5AEA*/
  ...
  rx_att_data = 0;
  write(spi_fd, &rx_att_data, 1); /*set S-ATT=0 as PE4302 powerup setting does not work*/

/* end of USB serial and SPI initial setting JA5AEA*/
  ...
  if((i2c_fd = open("/dev/i2c-0", O_RDWR)) >= 0)
  {
    ...
    if(ioctl(i2c_fd, I2C_SLAVE, ADDR_CODEC) >= 0)
    {
     ...
     /* JA5AEA: set audio codec clock */
        /* changed to 8 JA5AEA*/
        switch (atoi(argv[8])) {
        case 1: /* audio codec clock 12.288MHz */
            printf("audio codec clock 12.288MHz selected\n");
            break;
        case 2: /* audio codec clock 12.000MHz */
            i2c_write_addr_data8(i2c_fd, 0x10, 0x01);
            printf("audio codec clock 12.000MHz selected\n");
            break;
        default:
            printf("Usage: undefined audio codec clock argement\n");
            return EXIT_FAILURE;
            break;
        }
     /* end: JA5AEA: set audio codec clock */
        /* set activate register */
        i2c_write_addr_data8(i2c_fd, 0x12, 0x01);
        /* set power down register, JA5AEA: changed 0x41 to 0x40 to disable line input power down */
        i2c_write_addr_data8(i2c_fd, 0x0c, 0x40);
      }
    }
  }
...
void process_ep2(uint8_t *frame)
{
  ...
  uint16_t data;
  uint32_t data32;
  uint8_t ptt, preamp, att, boost, select, linein, linevol, xvtr, ps;/* after select added by JA5AEA*/
  uint8_t data8;
  ...
  switch(frame[0])
  {
    ...
    case 1:
      ...
      /* set output pins */
      ptt = frame[0] & 0x01;
      /* change 20dB and 10dB ATT to PS and XVTR activation by JA5AEA
      att = frame[3] & 0x03;
      */
      if ((frame[3] & 0x60) == 0x60) xvtr = 0x04;
      preamp = ptt | (*gpio_in & 1) ? 0 : (frame[3] & 0x04) >> 2 | (rx_att_data == 0);
      *gpio_out = (frame[2] & 0x1e) << 3 | ps << 3 | xvtr | preamp << 1 | ptt;

      /* set USB serial PTT JA5AEA */
      ...
      if (ptt_data != ptt)
      {
          ...
          {
          case 0:
              if (usb_serial >= 0) write(usb_serial, "RX;", 3);
              ptt_data = 0;
              break;
          case 1:
              if (usb_serial >= 0) write(usb_serial, "TX;", 3);
              ptt_data = 1;
              break;
          }
      }
      /* end: set USB serial PTT JA5AEA */
    ...
    case 5:
      ...
      *rx_freq[0] = (uint32_t)floor(freq / 125.0e6 * (1 << 30) + 0.5);
      ...
      if(freq_data[1] != freq)
      {
        ...
        /* set USB serial frequency JA5AEA*/
        char buffer[14];
        char beg[5] = "FA000";
        char end[1] = ";";
        sprintf(buffer, "%.5s%.8d%.1s", beg, freq_data[1], end);
        if (usb_serial >= 0) write(usb_serial, buffer, 14);
        /* end set USB serial frequency JA5AEA*/
    ...
    case 19:
      ...
      else
      {
       ...
       /* configure microphone boost replaced by JA5AEA*/
      if (i2c_codec)
          linein = frame[2] & 0x02;
      boost = frame[2] & 0x01;
      if (i2c_boost_data != boost)
      {
          ...
          printf("boost status changed\n");
          ioctl(i2c_fd, I2C_SLAVE, ADDR_CODEC);
          i2c_write_addr_data8(i2c_fd, 0x08, 0x14 + boost);
      }
      if (i2c_linein_data != linein)
      {
          i2c_linein_data = linein;
          if (linein)
          {
              printf("linein selected\n");
              ioctl(i2c_fd, I2C_SLAVE, ADDR_CODEC);
              i2c_write_addr_data8(i2c_fd, 0x08, 0x10);
          }
          else
          {
              printf("mic selected considering previous boot status\n");
              ioctl(i2c_fd, I2C_SLAVE, ADDR_CODEC);
              i2c_write_addr_data8(i2c_fd, 0x08, 0x14 + i2c_boost_data);
          }
      }
      break;
      /* end: configure microphone boost replaced by JA5AEA*/
      if(i2c_codec)
      {
        boost = frame[2] & 0x01;
        if(i2c_boost_data != boost)
    ...
    case 21:
      ...
      ps = ptt && (frame[2] & 0x40 >> 6); /* puresignal relay added by JA5AEA */
      
      /*OpenHPSDR controls both RX1 and RX2 S-ATT via C4 (0 - 31dB [4:0]) at C0 = "20" or "21". Open HPSDR should limit S-ATT maximum value as 31 as C4 is only five bits. JA5AEA */
      if (spi_fd) 
      {
      ...
      /* end: OpenHPSDR controls both RX1 and RX2 S-ATT via C4 (0 - 31dB [4:0]) at C0 = "20" or "21". Open HPSDR should limit S-ATT maximum value as 31 as C4 is only five bits. JA5AEA */
      ...
      /*added lince volume control JA5AEA*/
      linevol = frame[2] & 0x1f;
      if (i2c_codec)
      {
          if (i2c_linevol_data != linevol)
          {
              i2c_linevol_data = linevol;
              ioctl(i2c_fd, I2C_SLAVE, ADDR_CODEC);
              i2c_write_addr_data8(i2c_fd, 0x01, linevol);
          }
      }     
      /*end: added lince volume control JA5AEA*/
      break;
    case 22:
    case 23:
      if(i2c_misc)
...
static inline void cw_on()
{
  ...
  /* PTT on */
  *tx_rst |= 16; 
  if (usb_serial >= 0) write(usb_serial, "TX;", 3); /* added by JA5AEA */
  cw_ptt = 1;
  tx_mux[16] = 1;
  tx_mux[0] = 2;
  tx_mux_data = 1;
